...
def scrape_movie_details(page_url: str) -> dict:
    ...
    try:
        ...
        m = (
            re.search(r'\b(\d{1,2}:\d{2}:\d{2})\b', full_text) or
...
async def upload_video(tl_client, input_path: str, title: str, reply_to: int | None) -> bool:
    ...
    log.info(
        f"  Uploading {os.path.basename(input_path)} "
        f"({file_size/1024/1024:.1f} MB) | {width}x{height} | {duration}s"
    )
    ...
    try:
        ...
        log.info(f"  ✅ Uploaded: {os.path.basename(input_path)}")
        return True
    except Exception as e:
